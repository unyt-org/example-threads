...
const base32 = await import(
  "https://cdn.jsdelivr.net/npm/hi-base32@0.5.1/+esm"
);
...
class PublicKey {
  ...
  toString() {
    return btoa(
      "== ed25519v1-public: type0 ==\x00\x00\x00".concat(
        String.fromCharCode.apply(null, [...this.value]),
      ),
    );
  }
...
class PrivateKey {
  ...
  toString() {
    return btoa(
      "== ed25519v1-secret: type0 ==\x00\x00\x00".concat(
        String.fromCharCode.apply(null, [...this.value]),
      ),
    );
  }
...
async function generateOnionV3(
  keys: KeyPair | Promise<KeyPair> = generateKeys(),
): Promise<AddressData> {
  keys = await keys;
  ...
  const address = base32.encode(Array.from(decoded)).toLowerCase().concat(
    ".onion",
  );
